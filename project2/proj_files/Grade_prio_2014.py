...
# find any, please email class TA and I will
...
# September 23, 2013
#
# For the Priority Token Ring, each line in the output files should
# correspond to:
...
for fileNum in range(1, number+1):
	...
	while 1:
		...
		if len(fields) == 4:
			# Organize the entry
			entry = {"Source": fileNum, "Dest": int(fields[0]), "Prior": int(fields[1]), "Size": int(fields[2]), "Data": fields[3]}
...
for i in range(0, number):
	...
	for entry in destAns:
		...
		file.write(str(entry["Source"]) + "," + str(entry["Dest"]) + "," + str(entry["Prior"]) + "," + str(entry["Size"]) + "," + entry["Data"])
...
for fileNum in range(1, number + 1):
	...
	while 1:
		...
		fields = line.split(",")
		if len(fields) == 5:
			entry = {"Source": int(fields[0]), "Dest": int(fields[1]), "Prior": int(fields[2]), "Size": int(fields[3]), "Data": fields[4]}
